...
FatFs - Tiny FAT file system module  sweta, 2008

step 1- Defined make filesystem function,while defining 
	
         (i)Removed the statement  //if (drv >= _DRIVES) return FR_INVALID_DRIVE;because Tinyfatfs does not 
		  support multiple logical drives.
		  
	     (ii)Also removed LD2PD function as this function is required in multipartition drives.
		 FR_OK) return res;

Step 2 - Removed f_stat function for checking the file status.

Syep 3  Removed f_truncate function for truncating file.
 
Step 4 - Removed f_stat function for checking the file status.

Step 5 - Removed f_mkdir function for making a directory.

Step 6 - Removed f_chmod function for changing the file attributes.

Step 7 - Removed f_utime function for changing the file timestamp.
Step 8 - Removed f_rename function for renaming a file or directory.
